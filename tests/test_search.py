...
def test_search_requires_auth(client):
    assert client.get("/search?q=coffee").status_code == 401
...
def test_search_finds_and_ranks_by_text(authed_client):
    ...
    results = authed_client.get("/search?q=coffee").get_json()["results"]
...
def test_search_filter_by_who(authed_client):
    ...
    results = authed_client.get("/search?who=Alex").get_json()["results"]
...
def test_search_filter_by_date_range(authed_client):
    ...
    results = authed_client.get("/search?from=2026-05-20&to=2026-05-31").get_json()["results"]
...
def test_search_scoped_to_user(authed_client, app):
    ...
    assert other.get("/search?q=secret").get_json()["results"] == []
...
def test_search_natural_language_date(authed_client):
    ...
    res = authed_client.get("/search?q=what happened on 16 May 2026").get_json()
...
def test_search_nl_date_with_content(authed_client):
    ...
    names = [r["name"] for r in authed_client.get(
        "/search?q=dinner on 16 May 2026").get_json()["results"]]
...
def test_explicit_dates_override_nl_query(authed_client):
    ...
    res = authed_client.get(
        "/search?q=16 May 2026&from=2026-05-20&to=2026-05-20").get_json()
...
def test_search_reflects_edits_and_deletes(authed_client):
    ...
    # not yet present
    assert authed_client.get("/search?q=retrospective").get_json()["results"] == []
    ...
    assert authed_client.get("/search?q=retrospective").get_json()["results"][0]["id"] == event_id
